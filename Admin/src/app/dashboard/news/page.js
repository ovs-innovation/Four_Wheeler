...
import { Newspaper, Plus, Trash2, Edit, X, Save, Upload } from 'lucide-react';
...
export default function NewsManager() {
  ...
  const fetchNews = async () => {
    try {
      setLoading(true);
      const res = await adminApi.get('/news?limit=1000');
      if (res.data?.success) {
        setNews(res.data.data.news);
      }
    } catch (err) {
      console.error('Failed to load news articles:', err);
      toast.error('Failed to fetch news index.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEditModal = (item) => {
    ...
    reset({
      ...
      author: item.author || 'Editor-in-Chief',
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        toast.success('News banner uploaded successfully!');
      }
    } catch (err) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (editItem) {
        const res = await adminApi.put(`/news/${editItem._id}`, payload);
        if (res.data?.success) {
          setNews(news.map(n => n._id === editItem._id ? res.data.data : n));
          toast.success('News article specs saved.');
          setModalOpen(false);
        }
      } else {
        const res = await adminApi.post('/news', payload);
        if (res.data?.success) {
          setNews([res.data.data, ...news]);
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl font-sans font-extrabold text-white flex items-center gap-2 uppercase tracking-wider">
            <Newspaper className="text-[#D4AF37]" /> Manage Launch News
          </h1>
          <p className="text-xs text-[#6B7280] mt-1">Publish press releases, launch announcements, spy shots, and safety updates.</p>
        </div>
        <button
          onClick={openAddModal}
          className="bg-gradient-to-r from-[#D4AF37] to-[#F4B400] text-black font-extrabold text-xs py-2.5 px-4 rounded-[14px] shadow-lg hover:brightness-110 hover:shadow-[0_0_25px_rgba(212,175,55,0.25)] transition-all duration-300 flex items-center gap-1.5 cursor-pointer hover:-translate-y-0.5 active:translate-y-0"
        >
          ...
          <table className="w-full border-collapse text-left text-xs text-[#9CA3AF]">
            <thead>
              <tr className="border-b border-[#2A2A2A] text-[#6B7280] font-bold uppercase text-[9px] tracking-wider bg-[#111111]/50">
                <th className="p-4">Article Banner</th>
              ...
              {news.length > 0 ? (
                news.map((item) => (
                  <tr key={item._id} className="hover:bg-[#202020] transition-colors duration-300 odd:bg-transparent even:bg-[#171717]/10">
                    ...
                    <td className="p-4 text-[#9CA3AF] font-semibold">{item.author}</td>
                ...
                <tr>
                  <td colSpan="6" className="p-8 text-center text-[#6B7280] font-semibold">
                    No articles currently cataloged in the press registry database.
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 bg-[#0B0B0D]/80 backdrop-blur-sm flex items-center justify-center overflow-y-auto px-4 py-8">
          <div className="w-full max-w-2xl bg-[#171717] border border-[#2A2A2A] rounded-[22px] p-6 shadow-2xl space-y-6 my-auto font-sans">
            ...
            <div className="flex justify-between items-center pb-3 border-b border-[#2A2A2A]">
              <h3 className="font-sans font-extrabold text-sm text-white uppercase tracking-wider">
                {editItem ? 'Edit Press Release' : 'Publish New Article'}
              </h3>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 max-h-[65vh] overflow-y-auto pr-2">
              <div>
                <label className="block text-[10px] font-bold text-[#6B7280] uppercase tracking-wider mb-1.5">Article Title</label>
                <input
                  type="text"
                  placeholder="e.g. Royal Enfield Hunter 350 Electric Spotted testing in Chennai"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="e.g. hunter-350-electric-spotted-testing"
                    className="block w-full px-3 py-2.5 bg-[#151515] border border-[#303030] rounded-[14px] text-xs text-white placeholder-[#6B7280] focus:outline-none focus:border-[#D4AF37] focus:ring-1 focus:ring-[#D4AF37]/20 transition-all"
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="Spy Shots">Spy Shots</option>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-[#6B7280] uppercase tracking-wider mb-1.5">Author Name</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-[10px] font-bold text-[#6B7280] uppercase tracking-wider mb-1.5">Estimated Read Time</label>
                  <input
                    type="text"
              ...
              <div>
                ...
                <textarea
                  rows="2"
                  placeholder="Summarize the core theme of this press release..."
                  className="block w-full px-3 py-2.5 bg-[#151515] border border-[#303030] rounded-[14px] text-xs text-white placeholder-[#6B7280] focus:outline-none focus:border-[#D4AF37] focus:ring-1 focus:ring-[#D4AF37]/20 transition-all"
              ...
              <div>
                ...
                <textarea
                  rows="6"
                  placeholder="Write the full press release article context details..."
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-[#2A2A2A]">
                ...
                >
                  <Save size={13} /> {editItem ? 'Save Article' : 'Publish Article'}
                </button>
              </div>
            </form>
