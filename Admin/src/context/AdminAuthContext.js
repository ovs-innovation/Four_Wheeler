  ...
  useEffect(() => {
    const checkLoginState = async () => {
      if (typeof window !== 'undefined') {
        const savedToken = localStorage.getItem('cj_admin_token');
        if (savedToken) {
          try {
            setToken(savedToken);
  ...
  const login = async (email, password) => {
    try {
      ...
      if (res.data && res.data.success) {
        ...
        setIsAuthenticated(true);
        if (typeof window !== 'undefined') {
          localStorage.setItem('cj_admin_token', adminToken);
        }
      ...
      const errMsg = err.response?.data?.message || 'Invalid administrative credentials';
      toast.error(errMsg);
      return { success: false, error: errMsg };
    } finally {
      setLoading(false);
