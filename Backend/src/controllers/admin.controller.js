...
const getDashboardStats = async (req, res, next) => {
  try {
    const totalCars = await Car.countDocuments({ isDeleted: { $ne: true } });
    ...
    const electricCars = await Car.countDocuments({ $or: [{ fuelType: 'ELECTRIC' }, { isElectric: true }], isDeleted: { $ne: true } });
    const popularCars = await Car.countDocuments({ isPopular: true, isDeleted: { $ne: true } });
    ...
    res.status(200).json({
      ...
      data: {
        counts: {
          bikes: totalCars, // alias for frontend compatibility
          cars: totalCars,
          brands: totalBrands,
          ...
          news: totalNews,
          blogs: totalBlogs,
          electricBikes: electricCars, // alias
          electricCars,
          popularBikes: popularCars, // alias
          popularCars
        },
        recentEnquiries,
        recentUsers
